...
**象限顺序：** 起始象限 4 选 1 随机 + 环绕方向（顺时针/逆时针）2 选 1 随机，共 8 种起始配置。这样每次跨象限过渡都天然衔接相邻象限。

**单象限走法**（以右下象限为例，远离中心 = 右+下）：
...
| ↓ | 下 1 格 |
| 第 6 行 | 左 4 段 |
| 过渡 | 再横向 4 段，方向朝向下一个象限 → 进入相邻象限 |

其他象限走法对称：横向起始方向 = 远离中心的水平方向，纵向 1 格方向 = 远离中心的垂直方向。跨象限的 4 段方向由环绕顺序决定（例如右下 → 左下走"左"，右下 → 右上走"上"）。
