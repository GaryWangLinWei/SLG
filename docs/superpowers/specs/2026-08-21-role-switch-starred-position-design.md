...
代价分析：切账号无法选择落在哪个角色上（落在该账号最近使用的角色），而轮换每轮结束时该账号的最近使用角色都不是下一轮的目标，因此**每轮的跨账号 role 型切换都要真实执行一次角色切换**（一次登录等待），这是必要成本。唯一例外是首轮落点恰好等于目标（取决于手动留下的初始状态，不可预测）——此时角色切换为空操作，见 5.3 的空操作识别分支。
...
**重试幂等性**：重试时前端按当前逻辑状态重算步骤。若上一次尝试已完成账号切换、仅角色切换失败，重试会再次包含 `accountSwitch`——此时设备已在目标账号上，该步骤必须容忍"目标即当前账号"的情况（跳过或接受重选，不得进入"找不到目标→重启游戏"的失败分支）；`roleSwitch` 侧由 5.3 的空操作识别分支兜底（已在目标角色时干净退出并报成功）。
...
5. 等待 `btn_surelogin`（约 3s，时长实施时标定）：
   - **出现** → 确认登录 → `waitForCityAfterLogin` 等进城（真实切换）
   - **未出现** → 点击当前已激活角色无效果（游戏行为：不重新登录、停留原界面）→ 判定"已在目标角色"，**逐层关闭打开的 3 个界面**（角色管理 ✕ → 设置返回 → 头像菜单关闭）回城，报成功
...
- **真机 E2E**（dev 4 槽，模拟器手动验证）：
  ...
  - 已在目标角色时执行 roleSwitch → 空操作分支：等待超时后逐层关闭 3 个界面回城、报成功（可手动把 B 留在 ① 后跑 `[A, B①, B②]` 首轮构造）
